...
LOG_FILE = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'fetch_insert_auto.log')
...
def populate():
    try:
        ...
        a = f'/class/gp/creation_date/%3Enow-0.125/orderby/norad_cat_id/format/json'

        # gives the lates data from the past 12 h 
        b =  f'/class/gp/creation_date/%3Enow-0.5/orderby/norad_cat_id/format/json'
        ...
        with requests.Session() as session:
            ...
            resp = session.get(uriBase + requestCmdAction + b)
            resp.raise_for_status()  # Raise an error for bad response status
