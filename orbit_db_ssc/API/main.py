...
# This file is the main file for the API. It contains the FastAPI application and the endpoints for the API.
# There are lots of improvements and some endpoints that havent been tested yet at the bottom.
...
# a function that returns the given NORAD ID's data
...
# this was just a test endpoint to display the table limited to 10f the newest entries. 
...
# this endpoint returns the number of unique objects in the database.
@app.get("/count_rows")
async def count_rows():
    try:
        ...
        await close_connection(conn)
        return {"Number of unique objects managed in the database": count_result}
    except Exception as e:
        return {"error": str(e)}
...
#####################################################################################
# Search endpoint with optional start_date and end_date parameters


@app.get("/search")
async def search_object(norad_cat_id: int, start_date: Optional[datetime] = None, end_date: Optional[datetime] = None):
    try:
        # Connect to the database
        conn = await connect_to_database()
        
        # Execute SQL query to search for all data for the specified norad_cat_id based on insertion_date in descending order
        query = """
            SELECT * 
            FROM gp 
