...
# THIS SCRIPT IS USED TO FETCH DATA FROM SPACE-TRACK.ORG AND INSERT IT INTO A POSTGRESQL DATABASE.
### IT RUNS IN A CONTINUOUS LOOP, FETCHING DATA EVERY 60 SECONDS.
...
def configure_logging():
    ...
    formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s', datefmt='%Y-%m-%d %H:%M:%S')################ Used for finding out how far off 
    formatter.converter = time.gmtime  # Convert timestamps to UTC time ################################################### the fetch was from the current time.
...
def fetch_satellite_data(session, uriBase, requestCmdAction, any):
    ...
    # potential retry logic for fetching satellite data. NOT TESTED. NOT NEEDED. 
...
def populate():
    ...
    try:
        ...
        #################################################################################################################################################  
        populate = f'/class/gp/DECAY_DATE/null-val/orderby/norad_cat_id/format/json' ### THE INITAL POPULATION, USED ONCE ONLY TO POPULATE THE DATABASE when it has been created.
       ##################################################################################################################################################

        #####################################################################
        ## A SET OF QUERIES THAT CAN BE USED TO GET DATA FROM SPACE-TRACK.ORG
        # the only one necessary for this script is the query variable.


        # I have used these queries after working and updating the script to get data that might have been missed when the script wasent running.
        ...
        query = f'/class/gp/CREATION_DATE/%3Enow-0.0068/orderby/CREATION_DATE%20asc/format/json' # gives data from the last ca 10 minutes. in creation_date order.
