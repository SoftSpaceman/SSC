...
### IT HAS NOT BEEN RUN ON TJE orbital_info_db DATABASE.
...
# LOG_FILE = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'insert_gpdata2.log')
...
LOG_FILE = os.path.join(LOG_DIRECTORY, 'insert_orbital_info_db.log')
file_handler = RotatingFileHandler(LOG_FILE, backupCount=3)
formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')
file_handler.setFormatter(formatter)
logger.addHandler(file_handler)
...
def populate():
    try:
        ...
        with requests.Session() as session:
            ...
            resp = session.get(uriBase + requestCmdAction + a)
            resp.raise_for_status()  # Raise an error for bad response status
