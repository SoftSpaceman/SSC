...
logging.basicConfig(filename='GP_populate_twice.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
def store_data_in_postgresql(data):
    num_inserted_gp_file = 0
    num_inserted_gp_file_historical = 0
    conn = None
    try:
        ...
        # Insert data into PostgreSQL table gp_file and gp_file_historical
        for item in data:
            try:
                    ...
                    num_inserted_gp_file_historical += 1
    ...
    return num_inserted_gp_file, num_inserted_gp_file_historical
...
with requests.Session() as session:
    try:
        ...
        if fetched_data:
            ...
            logging.info("Number of objects inserted: %s", num_inserted)
        else:
