...
def configure_logging():
    ...
    max_bytes = 10 * 1024 * 1024  # 10 MB in bytes
    file_handler = RotatingFileHandler(LOG_FILE, maxBytes=max_bytes, backupCount=4)
...
def populate():
    ...
    try:
        ...
        x = f'/class/gp/creation_date/%3Enow-0.0069444/orderby/norad_cat_id/format/json' ######### 0.0069444 days = data from now and 10 minuts back.
        #################################################################################
        y = f'/class/gp/creation_date/%3Enow-0.0208333/orderby/norad_cat_id/format/json'######### 0.0208333 days = data from now and 30 minuts back.
        #################################################################################
        a = f'/class/gp/creation_date/%3Enow-0.0416667/orderby/norad_cat_id/format/json'######### 0.0416667 days = data from now and 1 hour back.
        #################################################################################
        ...
        fetched_data = fetch_satellite_data(session, uriBase, requestCmdAction, a) ######## Insertion query here.
                                                                                    ##### If you change query here, dot forget to change the parameter in fetch_satellite_data function.
...
if __name__ == "__main__":
    ...
    while True:
        try:
            populate()
            # Sleep for 60 x 8  seconds before running again
            time.sleep(480)
