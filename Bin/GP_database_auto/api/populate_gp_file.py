...
# Set up logging
logging.basicConfig(filename='database_creation.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
def store_data_in_postgresql(data, host, dbname, user, password, port):
    num_inserted = 0
    conn = None
    try:
        ...
        for item in data:
            try:
                ...
                logging.error(f"Error occurred with item {item}: {e}")
                raise e
...
def main():
    ...
    with requests.Session() as session:
        try:
            ...
            if fetched_data:
                num_inserted = store_data_in_postgresql(fetched_data, host, dbname, user, password, port)
...
# logging.basicConfig(filename='GP_populate_auto.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
