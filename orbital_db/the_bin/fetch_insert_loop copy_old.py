...
# THIS COPY OF SCRIPT IS MEANT TO TRY THE FUNCTION THAT IF LIMIT IS REACHED, THE script will wait 1h and try agian. 


######################################################################
######################################################################

#################### THIS ONE SCRIPT COME SFROM A OLD VERISON 
...
def configure_logging():
    ...
    LOG_DIRECTORY = os.path.join(script_directory, "fetch_log2")  # Path to fetch_log folder
    if not os.path.exists(LOG_DIRECTORY):
        os.makedirs(LOG_DIRECTORY)
    LOG_FILE = os.path.join(LOG_DIRECTORY, 'automatic_insert_orbital_info_db2.log')
...
#################################################################################################################
####################### THIS IS A NEW FUNCTION ADDED TODAY 16/04/2021 ###########################################
#################### THE FUNCTION SHALL ADD A KIND OF FAIL SAFE THAT, IF THE REQUEST IT TOO BIG, IT WILL WAIT 1 HOUR AND TRY AGAIN.

# This function fetches satellite data from space-track.org using a provided session, base URI, command action, and additional query parameters.
# Parameters:
# session: A requests session object for making HTTP requests.
# uriBase: The base URI for the space-track.org API.
# requestCmdAction: The command action for the API request.
# any: Additional query parameters to be appended to the request URL.


# "If a `requests.exceptions.ConnectionError` occurs, it may be due to the server rejecting incoming connections or dropping existing ones, 
# possibly because the requested data is too large. This can result in disconnection from the server. 
# When attempting to reconnect, the client needs to translate the server's domain name into an IP address using DNS. 
# If there are issues with DNS resolution, such as network congestion or server problems, an `OSError` indicating a DNS resolution error may occur. 
# To handle this, the script pauses for __ seconds = __ minutes before retrying the operation."
...
def fetch_satellite_data(session, uriBase, requestCmdAction, any):
    ...
    try:
        ...
        if isinstance(e, requests.exceptions.ConnectionError) and isinstance(e.__cause__, OSError):
            # Pause execution for a period before retrying
            logging.info("Pausing script for 60 seconds before retrying...")
            time.sleep(60)  # Pause for 60 seconds or 3600 seconds = 1 hour
            # Retry the operation
            return fetch_satellite_data(session, uriBase, requestCmdAction, any)
        else:
            ...
            for obj in fetched_data:
                ...
                if count == 0:
                    try:
                        ...
                        logging.info(f"Inserted NORAD_CAT_ID: {obj['NORAD_CAT_ID']} with SpaceTrack creation date: {obj['CREATION_DATE']}")
                    ...
                    skipped_count += 1
                    logging.warning(f"Skipped duplicate NORAD_CAT_ID: {obj['NORAD_CAT_ID']} with SpaceTrack creation date: {obj['CREATION_DATE']}")
            conn.commit()
...
def populate():
    ...
    try:
        ...
        minute = f'/class/gp/creation_date/%3Enow-0.0006944/orderby/norad_cat_id/format/json' ######### 0.00069444 days = data from now and 1 minuts back. TRY IT OUT!

        two_min = f'/class/gp/creation_date/%3Enow-0.0013889/orderby/norad_cat_id/format/json' ######### 0.0013889 days = data from now and 2 minuts back. TRY IT OUT!

        z = f'/class/gp/creation_date/%3Enow-0.0034722/orderby/norad_cat_id/format/json' ######### 0.00069444 days = data from now and 5 minuts back. TRY IT OUT!
        ...
        fetched_data = fetch_satellite_data(session, uriBase, requestCmdAction, z) ######## Insertion query here.
                                                                                #    
